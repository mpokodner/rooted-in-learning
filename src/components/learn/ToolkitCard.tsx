...
> = {
  ...
  intermediate: { bg: 'rgba(213, 163, 114, 0.2)', color: 'var(--terracotta-dark)' },
...
export default function ToolkitCard({ resource }: { resource: ToolkitResourceCard }) {
    ...
    >
      ...
      {resource.featuredImage?.asset && (
        <div style={{ position: 'relative', aspectRatio: '16 / 9', overflow: 'hidden' }}>
          <Image
            src={urlFor(resource.featuredImage).width(600).height(340).quality(80).url()}
          ...
          {resource.difficultyLevel && (
            <span
              style={{
                ...
                fontWeight: 500,
                backgroundColor:
                  difficultyStyles[resource.difficultyLevel]?.bg || 'var(--beige-bg)',
                color:
                  difficultyStyles[resource.difficultyLevel]?.color || 'var(--text-muted)',
              }}
            >
